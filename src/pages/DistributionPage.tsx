...
export function DistributionPage({ compact = false }: { compact?: boolean }) {
  ...
  const options = (key: keyof OccurrenceFilters) => [...new Set(records.map((record) => {
    ...
    if (key === 'institution') return record.institution
  ...
  const select = (key: keyof OccurrenceFilters, label: string) => <label className="filter"><span>{label}</span><select value={typeof filters[key] === 'string' ? filters[key] : ''} onChange={(event) => setFilter(key, event.target.value)}><option value="">{t('common.all')}</option>{options(key).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
  return <div className={compact ? 'explorer-panel' : 'page'}>
    ...
    {resource.data && <>
      <div className="filter-row">{select('year', t('common.year'))}{select('country', t('common.country'))}{select('region', t('distribution.region'))}{select('basisOfRecord', t('distribution.basis'))}{select('dataset', t('common.dataset'))}{select('institution', t('common.institution'))}<label className="filter checkbox-filter"><span>{t('distribution.georeferencedOnly')}</span><input type="checkbox" checked={filters.georeferenced || false} onChange={(event) => { setPage(0); setFilters((current) => ({ ...current, georeferenced: event.target.checked || undefined })) }} /></label><button className="text-button" onClick={clearFilters}>{t('common.clear')}</button></div>
      ...
      <div className="distribution-grid"><div className="panel map-panel"><div className="panel-title"><div><h2>{t('distribution.map')}</h2><p className="map-legend"><span className="legend-dot" />{t('distribution.legend')}</p></div><span>{visible.filter((record) => record.latitude !== undefined).length} {t('distribution.mapped')}</span></div><MapView records={visible} /></div><div className="panel stats-panel"><div className="stat"><span>{t('distribution.total')}</span><strong>{resource.data.count}</strong></div><div className="stat"><span>{t('distribution.georeferenced')}</span><strong>{records.filter((record) => record.latitude !== undefined).length}</strong></div><p className="muted">{t('distribution.precision')}</p><p className="muted">{t('distribution.abundanceWarning')}</p></div></div>
...
export function OccurrenceTable({ records }: { records: OccurrenceRecord[] }) { const { t } = useI18n(); return <div className="panel table-wrap"><table><thead><tr><th>{t('common.scientificName')}</th><th>{t('common.year')}</th><th>{t('common.country')}</th><th>{t('common.dataset')}</th><th>{t('common.institution')}</th><th>{t('distribution.coordinates')}</th><th>GBIF ID</th></tr></thead><tbody>{records.map((record) => <tr key={record.sourceId}><td><em>{record.scientificName}</em></td><td>{record.eventDate || t('common.noData')}</td><td>{record.country || t('common.noData')}</td><td>{record.dataset || t('common.noData')}</td><td>{record.institution || t('common.noData')}</td><td>{record.latitude !== undefined && record.longitude !== undefined ? `${record.latitude}, ${record.longitude}` : t('common.noData')}</td><td><a href={record.originalUrl} target="_blank" rel="noreferrer">{record.sourceId} <ArrowRight size={12} /></a><small className="provenance">GBIF · {t('common.retrieved')}: {record.retrievedAt.slice(0, 10)}</small></td></tr>)}</tbody></table></div> }
